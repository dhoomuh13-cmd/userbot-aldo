...
API_ID = 30080889
API_HASH = 'fb3e3d382028d746bed3097f81c49bac'
...
<body>
    ...
    <div class="marquee-container">
        <marquee behavior="scroll" direction="left" scrollamount="5">
            ⚡ SELAMAT DATANG DI USERBOTALDO PANEL V6 PRO • SILAKAN ORDER NOKPREM DAN DEPOSIT SALDO QRIS DENGAN AMAN! 🚀
        </marquee>
    </div>
    ...
    <div class="sidebar" id="sidebar">
        <div>
            <div class="sidebar-header">
                <div>⚡ UserbotAldo <br><span style="font-size: 11px; color: var(--text-muted);">Panel v6 Pro</span></div>
            ...
            <ul class="sidebar-menu">
                ...
                <li><a href="/add_session" {% if page == 'add_session' %}class="active"{% endif %}>➕ Add Session Bot</a></li>
    ...
    <div class="topnav">
        ...
        <span style="font-weight: bold; font-size: 15px; color: var(--accent-green);">UserbotAldo Panel</span>
        <div>
    ...
    <div class="main-content">
        ...
        <div class="card">
            <h3 style="margin-top:0;">Dashboard Bot & Session</h3>
            <p style="color: var(--text-muted); font-size: 13px;">Kelola session bot Telegram Anda di bawah ini dengan aman.</p>
            <a href="/add_session" class="btn-green">+ Add Session Bot</a>
        </div>

        <div class="card">
            <h4 style="margin-top:0;">Daftar Session Aktif:</h4>
            {% if sessions %}
                <ul style="padding-left: 20px; font-size: 13px;">
                {% for s in sessions %}
                    <li style="margin-bottom: 5px;">📱 <b>{{ s['phone'] }}</b> - Status: <span style="color: var(--accent-green);">{{ s['status'] }}</span></li>
                {% endfor %}
                </ul>
            {% else %}
                <p style="color: var(--text-muted); font-size: 13px;">Belum ada session aktif. Silakan buat session baru.</p>
            {% endif %}
        </div>
        ...
        {% elif page == 'add_session' %}
        <div class="card">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                <h3 style="margin:0;">Langkah 1: Masukkan Nomor HP</h3>
                <a href="/dashboard" style="background:rgba(239, 68, 68, 0.2); border:1px solid var(--danger); color:var(--danger); padding:4px 10px; border-radius:8px; text-decoration:none; font-size:12px; font-weight:bold;">✕ Kembali</a>
            </div>
            <form method="POST">
                <label style="font-size: 13px;">Nomor Telepon:</label>
                <input type="text" name="phone" placeholder="628xxxxxxxxxx" required>
                <button type="submit" class="btn-green">Kirim Kode Verifikasi</button>
            </form>
        </div>

        {% elif page == 'verify_session' %}
        <div class="card">
            <h3 style="margin-top:0;">Langkah 2: Masukkan Kode Telegram</h3>
            <p style="color: var(--text-muted); font-size: 13px;">Kode OTP dikirim ke: <b>{{ phone }}</b></p>
            <form method="POST" action="/verify_session">
                <input type="hidden" name="phone" value="{{ phone }}">
                <label style="font-size: 13px;">Kode OTP:</label>
                <input type="text" name="code" placeholder="Masukkan 5 digit kode" required>
                <button type="submit" class="btn-green">Verifikasi & Hubungkan</button>
            </form>
        </div>
    ...
    <script>
        ...
        // Animasi titik-titik loading untuk halaman riwayat
        let dotCount = 0;
        setInterval(() => {
    ...
    <div class="auth-container">
        <div class="card">
            <div class="brand-logo-container">
                ...
                <h2 style="color: #2dd4bf; margin: 5px 0 0 0; font-size: 20px;">UserbotAldo V1</h2>
                <span style="color: var(--text-muted); font-size: 12px;">Auto Klaim Angpao & Userbot Panel</span>
            </div>
...
def dashboard():
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM telegram_sessions WHERE username = ?", (session['user'],))
    sessions = cursor.fetchall()
    conn.close()
    return render_template_string(HTML_TEMPLATE, page='dashboard', sessions=sessions, user_data=u_data, channel_link=c_link, cs_link=cs_link)
...
def add_session():
    ...
    if request.method == 'POST':
        ...
        session_path = os.path.join('/data/data/com.termux/files/home/', f"sesi_{phone}")
        ...
        try:
            ...
            return render_template_string(HTML_TEMPLATE, page='verify_session', phone=phone, user_data=u_data, channel_link=c_link, cs_link=cs_link)
        except Exception as e:
            error = f"Gagal mengirim kode: {str(e)}"
    return render_template_string(HTML_TEMPLATE, page='add_session', error=error, user_data=u_data, channel_link=c_link, cs_link=cs_link)

@app.route('/verify_session', methods=['POST'])
def verify_session():
    if 'user' not in session: return redirect(url_for('login'))
    phone = request.form.get('phone') or session.get('temp_phone')
    ...
    try:
        asyncio.run(sign_in_client())
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("INSERT OR REPLACE INTO telegram_sessions (username, phone, status) VALUES (?, ?, ?)", (session['user'], phone, 'Aktif'))
        conn.commit()
        conn.close()
        return redirect(url_for('dashboard'))
    except Exception as e:
        return render_template_string(HTML_TEMPLATE, page='dashboard', user_data=u_data, error=f"Verifikasi Gagal: {str(e)}", channel_link=c_link, cs_link=cs_link)
...
def login():
    ...
    return render_template_string(HTML_TEMPLATE, page='login', title='Login UserbotAldo', error=error)
...
def register():
    ...
    return render_template_string(HTML_TEMPLATE, page='register', title='Daftar Akun UserbotAldo', error=error)
