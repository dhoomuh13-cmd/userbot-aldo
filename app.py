...
def init_db():
    ...
    cursor.execute("INSERT OR IGNORE INTO settings (key, value) VALUES ('dj_poster', 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=500&auto=format&fit=crop&q=80')")
    conn.commit()
    conn.close()
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div style="padding: 15px; border-top: 1px solid var(--border-color);">
            ...
            <a href="/logout" onclick="localStorage.clear();" style="color: var(--danger); text-decoration: none; font-weight: bold; font-size: 12px;">🚪 Logout</a>
        </div>
    </div>
    ...
    <div class="main-content">
        ...
        <div class="card" style="border: 1px solid var(--accent-green); background: linear-gradient(135deg, rgba(20, 184, 166, 0.12), rgba(18, 25, 38, 0.95));">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
                <div style="display: flex; align-items: center; gap: 12px;">
                    <img src="{{ dj_poster }}" alt="DJ Poster" style="width: 55px; height: 55px; object-fit: cover; border-radius: 10px; border: 2px solid var(--accent-green); box-shadow: 0 0 12px rgba(20, 184, 166, 0.4);">
                    <div>
                        <div style="font-size: 15px; font-weight: bold; color: var(--accent-green);">DJ BLACKHOLE ALDO</div>
                        <div style="display: inline-block; background: rgba(20, 184, 166, 0.2); color: var(--accent-green); font-size: 10px; font-weight: bold; padding: 2px 6px; border-radius: 4px; margin-top: 3px; border: 1px solid var(--accent-green);">LIVE</div>
                    </div>
                </div>
                <button type="button" id="playPauseBtn" onclick="togglePlayMusic()" style="background: var(--accent-green); color: white; border: none; padding: 6px 14px; border-radius: 8px; font-size: 12px; cursor: pointer; font-weight: bold;">Pause</button>
            </div>
            
            <div style="background: rgba(6, 9, 19, 0.5); padding: 12px; border-radius: 10px; border: 1px solid var(--border-color);">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    <span style="font-size: 13px; color: var(--text-muted); font-weight: bold;">Dashboard Volume:</span>
                    <span id="volumeValueText" style="font-size: 12px; color: var(--accent-green); font-weight: bold;">30%</span>
                </div>
                <input type="range" id="volumeSlider" min="0" max="1" step="0.05" value="0.3" style="width: 100%; margin: 0; cursor: pointer; accent-color: var(--accent-green);">
            </div>
        </div>
        ...
        <div class="card">
            ...
            <form method="POST">
                ...
                <label style="font-size: 13px;">URL Poster DJ BLACKHOLE ALDO:</label>
                <input type="text" name="dj_poster" value="{{ dj_poster }}">
    ...
    <script>
        ...
        if (bgMusic) {
            let savedTime = localStorage.getItem('aldoAudioTime');
            let savedVolume = localStorage.getItem('aldoAudioVolume');
            let savedState = localStorage.getItem('aldoAudioState');

            if (savedVolume !== null) {
                bgMusic.volume = parseFloat(savedVolume);
                if (volumeSlider) volumeSlider.value = savedVolume;
                if (volumeValueText) volumeValueText.innerText = Math.round(savedVolume * 100) + '%';
            } else {
                bgMusic.volume = 0.3;
            }
            ...
            if (savedState === 'pause') {
                ...
                bgMusic.play().catch(error => {
                    console.log("Autoplay diblokir browser, klik layar untuk mengaktifkan musik.");
                });
            ...
            setInterval(() => {
                if (!bgMusic.paused) {
                    localStorage.setItem('aldoAudioTime', bgMusic.currentTime);
                    localStorage.setItem('aldoAudioState', 'play');
                }
            }, 500);
        }
        ...
        if (volumeSlider) {
            volumeSlider.addEventListener('input', function() {
                if (bgMusic) {
                    ...
                    localStorage.setItem('aldoAudioVolume', volumeSlider.value);
                    if (volumeValueText) volumeValueText.innerText = Math.round(volumeSlider.value * 100) + '%';
                }
            });
        }
        ...
        function togglePlayMusic() {
            ...
            if (bgMusic.paused) {
                ...
                bgMusic.pause();
                if (playPauseBtn) playPauseBtn.innerText = "Play";
                localStorage.setItem('aldoAudioState', 'pause');
            }
        }
...
def get_settings():
    ...
    return (
        ...
        data.get('cs_link', ''), 
        data.get('qris_image', ''), 
        data.get('dj_poster', 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=500&auto=format&fit=crop&q=80')
    )
...
def dashboard():
    if 'user' not in session: return redirect(url_for('login'))
    c_link, cs_link, _, dj_p = get_settings()
    u_data = get_user_data(session['user'])
    return render_template_string(HTML_TEMPLATE, page='dashboard', user_data=u_data, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def restock_nokprem():
    ...
    return render_template_string(HTML_TEMPLATE, page='restock_nokprem', user_data=u_data, success_msg=success_msg, error=error, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def owner_deposits():
    ...
    conn.close()
    return render_template_string(HTML_TEMPLATE, page='owner_deposits', deposits=deposits, user_data=u_data, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def order_nokprem():
    ...
    conn.close()
    return render_template_string(HTML_TEMPLATE, page='order_nokprem', products=products, user_data=u_data, error=error, success_msg=success_msg, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def riwayat_pesanan_page():
    ...
    return render_template_string(HTML_TEMPLATE, page='riwayat_pesanan', pesanan=pesanan, user_data=u_data, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def deposit_saldo():
    ...
    c_link, cs_link, _, dj_p = get_settings()
    u_data = get_user_data(session['user'])
    return render_template_string(HTML_TEMPLATE, page='deposit', user_data=u_data, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)

@app.route('/deposit/qris', methods=['POST'])
def deposit_qris():
    if 'user' not in session: return redirect(url_for('login'))
    c_link, cs_link, qris_img, dj_p = get_settings()
    u_data = get_user_data(session['user'])
    try:
        ...
        conn.commit()
        conn.close()
        return render_template_string(HTML_TEMPLATE, page='deposit_qris', amount=amount, qris_image=qris_img, user_data=u_data, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
    except ValueError:
...
def game_bomb_page():
    ...
    c_link, cs_link, _, dj_p = get_settings()
    u_data = get_user_data(session['user'])
    return render_template_string(HTML_TEMPLATE, page='game_bomb', user_data=u_data, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def game_dice_page():
    ...
    c_link, cs_link, _, dj_p = get_settings()
    u_data = get_user_data(session['user'])
    return render_template_string(HTML_TEMPLATE, page='game_dice', user_data=u_data, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def leaderboard():
    ...
    conn.close()
    return render_template_string(HTML_TEMPLATE, page='leaderboard', user_data=u_data, leaders=leaders, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def livechat():
    ...
    conn.close()
    return render_template_string(HTML_TEMPLATE, page='livechat', chats=chats, user_data=u_data, channel_link=c_link, cs_link=cs_link, dj_poster=dj_p)
...
def settings():
    ...
    if request.method == 'POST' and session.get('role') == 'owner':
        ...
        new_dj_p = request.form['dj_poster']
        conn = get_db()
        cursor = conn.cursor()
        ...
        cursor.execute("REPLACE INTO settings (key, value) VALUES ('dj_poster', ?)", (new_dj_p,))
        conn.commit()
        conn.close()
        success = True
        c_link, cs_link, qris_img, dj_p = new_c, new_cs, new_qris, new_dj_p
    return render_template_string(HTML_TEMPLATE, page='settings', user_data=u_data, channel_link=c_link, cs_link=cs_link, qris_image=qris_img, dj_poster=dj_p, success=success)
