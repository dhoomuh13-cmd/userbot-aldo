...
<body>
        ...
        <div class="card" style="border: 1px solid var(--accent-green); background: linear-gradient(135deg, rgba(20, 184, 166, 0.12), rgba(18, 25, 38, 0.95));">
            <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px;">
                <div style="display: flex; align-items: center; gap: 12px;">
                    <video src="https://assets.mixkit.co/videos/preview/mixkit-dj-playing-music-in-a-club-41290-large.mp4" autoplay loop muted playsinline style="width: 55px; height: 55px; object-fit: cover; border-radius: 10px; border: 2px solid var(--accent-green); box-shadow: 0 0 12px rgba(20, 184, 166, 0.4);"></video>
                    <div>
                        <div style="font-size: 15px; font-weight: bold; color: var(--accent-green);">🎵 Background DJ Player</div>
                        <div style="font-size: 11px; color: var(--text-muted);">Putar terus tanpa keulang saat pindah menu</div>
                    </div>
                </div>
    ...
    <script>
        // Logika Persistent Audio & LocalStorage (Menghindari Audio Keulang Saat Pindah Halaman)
        ...
        if (bgMusic) {
            // Ambil memori waktu lagu & volume terakhir dari browser
            let savedTime = localStorage.getItem('aldoAudioTime');
            let savedVolume = localStorage.getItem('aldoAudioVolume');
            let savedState = localStorage.getItem('aldoAudioState'); // 'play' atau 'pause'
            ...
            // Atur status putar/jeda berdasarkan memori sebelumnya
            if (savedState === 'pause') {
                bgMusic.pause();
            ...
            // Simpan detik lagu secara berkala tiap 500ms
            setInterval(() => {
                if (!bgMusic.paused) {
